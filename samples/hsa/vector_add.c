...
int main(int argc, char** argv) {
   ...
   */
  hsa_queue_t* queue;
  err = hsa_queue_create(agent, queue_size, HSA_QUEUE_TYPE_SINGLE, NULL, NULL,
  ...
  FILE* hsaco_file = fopen("vector_add.hsaco", "r");
  int fd = fileno(hsaco_file);
  err = hsa_code_object_reader_create_from_file(fd, &code_reader);
  check(Create hsaco reader, err);

  /*
   * Load code object
   */
  err = hsa_executable_load_agent_code_object(executable, agent, code_reader,
                                              "", NULL);
  ...
  err = hsa_executable_get_symbol(executable, NULL, "vector_add", agent, 0,
                                  &symbol);
  ...
  char* in1;
  err = hsa_memory_allocate(global_region, GLOBAL_SIZE * 4, (void**)&in1);
  check(ALLOCATING FROM GLOBAL REGION, err);
  memset(in1, 1, GLOBAL_SIZE * 4);

  char* in2;
  err = hsa_memory_allocate(global_region, GLOBAL_SIZE * 4, (void**)&in2);
  check(ALLOCATING FROM GLOBAL REGION, err);
  memset(in1, 2, GLOBAL_SIZE * 4);
  ...
  err = hsa_memory_free(in1);
  check(Releasing memory allocated before, err);
  err = hsa_memory_free(in2);
  check(Releasing memory allocated before, err);
